...
export class BehaviorEngine {
  ...
  /**
   * Starts a periodic wall-clock sweep that force-closes any window whose
   * entity has gone quiet. Without this, a window only closes when a *new*
   * event arrives for that same entity — meaning a process that does
   * something malicious and then exits or goes idle would never cross the
   * detection floor. Call once at startup; returns a stop function.
   */
  startAutoFlush(intervalMs = 15_000): () => void {
    const id = setInterval(() => this.flush(Date.now()), intervalMs);
